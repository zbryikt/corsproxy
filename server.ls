require! <[chokidar http fs path jade stylus ./cors]>
...
test-cgi = (req, res) ->
  ...
  res.writeHead 200, {"Content-type": "text/html"}
  res.end JSON.stringify({method: req.method, form: data})

route-table = {"/sample-cgi": sample-cgi, "/test/": test-cgi}

server = (req, res) ->
  if /^\/x\//.exec req.url => return cors req.url.slice(3), res, req

  url = req.url - /[?#].*$/
  # custom server side script
  if url of route-table => return route-table[url] req, res
  ...
  if type == \ls =>
    if /src\/ls/.exec src =>
      try
        ...
        if !fs.exists-sync(\static/js) => mkdir-recurse \static/js
